...
class NetBarrier { // Mainly about connections within client nodes
 public:
  ...
  bool EndExperiment(std::vector<work_unit> &w, struct cstat_raw *csr) { // stat collection + whatever gathersamples does
    if (is_leader_) {
      for (auto &c : conns) {
 ...
 private:
  ...
  void GatherSamples(std::vector<work_unit> &w) { // check this out later
...
std::vector<work_unit> ClientWorker( // actually does the memcached workload generation, populates buffer, sends RPC request per work unit, and has a receiving thread for collecting credit and latency data  
  ...
  auto th = rt::Thread([&] {
    char resp[4096];
    struct MemcachedHdr *hdr; // Must be changed to Payload
    ...
    while (true) {
      ...
      if (ret != static_cast<ssize_t>(sizeof(MemcachedHdr) + hdr->total_body_length)) { // Must be changed to Payload
...
std::vector<work_unit> RunExperiment(
  ...
  for (int i = 0; i < threads; ++i) {
    th.emplace_back(rt::Thread([&, i] {
      ...
      samples[i].reset(new std::vector<work_unit>(std::move(v))); // Transfer the work vector to a new pointer samples[i] uses
    }));
  }
...
void ClientHandler(void *arg) { // executable for client collects parameters, which are sent to agent handler, and CSV outputs are generated by client too
  ...
  std::string wname; // unused variable + uses memcached types. Conditional logic removable
  if (wtype == 1)
