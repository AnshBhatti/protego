...
std::vector<Packet> GenerateWork(Arrival a, double cur_us,
  ...
  while (true) {
    ...
    w.emplace_back(Packet{0, rand(), 0, 0, 0, 0, 0, 0});
...
std::vector<Packet> ClientWorker( // actually does the memcached workload generation, populates buffer, sends RPC request per work unit, and has a receiving thread for collecting credit and latency data  
  ...
  std::cout << "Entering client worker\n";
  ...
  auto th = rt::Thread([&] {
    ...
    while (true) {
      ...
      // hdr = reinterpret_cast<struct MemcachedHdr *>(resp); // Must be changed to Payload
      // ntoh(hdr);
      pld = reinterpret_cast<struct Payload *>(resp);
      ntoh(pld);

      if (ret != static_cast<ssize_t>(sizeof(Payload))) { // Must be changed to Payload
        if (ret == 0 || ret < 0) break;
        panic("read failed, ret = %ld", ret);
      }
      uint64_t now = microtime();
      // NEED TO FIGURE OUT WHAT TO DO WITH RECEIVED PACKET. DOES RANDOMNESS REALLY MATTER WHEN I JUST CALCULATE CURRENT TIME?
      // I think we can use pld->index to refer to that specific payload, and gather latency and credit data.

      // /* Below block is memcached specific. Not sure what "opaque" is supposed to represent, 
      //   but need to figure this out in terms of Silo Payload */
      // uint32_t idx = hdr->opaque;
      uint32_t idx = pld->index; 
      ...
      // TODO: add latency data, we can also add the window field to Packet to accomodate credits
  ...
  std::cout << "Receiver thread generated\n";
  ...
  auto wsize = w.size();
  // char buf[4096];
  struct MemcachedHdr *hdr; // This must be changed to Payload
  struct Payload *pld;
  // int buflen;
  // char value[kValueLen];
  std::cout << "Entering sending loop\n";
  for (unsigned int i = 0; i < wsize; ++i) {
    std::cout << "just before one barrier\n";
    fflush(stdout);
    barrier();
    std::cout << "just one barrier\n";
    auto now = steady_clock::now();
    barrier();
    // below, target_starts could also be actual_starts instead depending on what they intend, but I think it's unlikely
    ...
    timings[i] = microtime();
    std::cout << "Assigning packet to payload\n";
    // packet to payload conversion
    // w[i].actual_start = now;
    pld->work_iterations = w[i].work_iterations;
    pld->index = i;
    pld->randomness = w[i].randomness;
    hton(pld);
    /* Since we are just concerned with sending the payload, the below conditional logic must be entirely replaced */
    // if (wtype == 1) { // SET
    ...
    // Send an RPC request.
    // Aren't we supposed to send client demand here? Also how are we not using srpc_send here? But c is an rpcClient object, so I am assuming that's taken care of
    // ssize_t ret = c->Send(buf, buflen, w[i].hash, nullptr); // ERIC
    std::cout << "GOING TO SEND PACKET\n";
    ssize_t ret = c->Send(pld, sizeof(pld), i, nullptr); // Note, likely to be buggy, im not sure what's really supposed to go for size
    if (ret == -ENOBUFS) continue;
    if (ret != static_cast<ssize_t>(sizeof(pld)))
...
std::vector<Packet> RunExperiment(
  ...
  for (int i = 0; i < threads; ++i) {
    ...
    std::cout << "I did connect to server\n";
  ...
  sstat_raw s1, s2;
  shstat_raw sh1, sh2;
  std::cout << "Going to compute stats\n";
  if (!b || b->IsLeader()) {
    s1 = ReadRPCSStat();
    sh1 = ReadShenangoStat();
  }
  std::cout << "Stats computed\n";
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 4) {
    std::cerr << "usage: [alg] [cfg_file] client [#threads] [remote_ip] [npeers]"
        << std::endl;
    return -EINVAL;
  }
  ...
  total_agents += std::stoi(argv[6], nullptr, 0);
